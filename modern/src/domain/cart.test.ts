...
describe("cart domain", () => {
  it("adds immutable lines and combines an existing product", () => {
    const first = cartReducer(EMPTY_CART, {
      type: "add",
      product,
      quantity: 2,
    });
  ...
  it("refuses invalid quantities and stock overflow", () => {
    ...
    expect(cartReducer(state, { type: "add", product, quantity: 1 })).toBe(
      state,
    );
    expect(
      cartReducer(state, {
  ...
  it("derives count and monetary total instead of storing them independently", () => {
    ...
    expect(deriveCartSummary(state)).toEqual({
      distinctLines: 2,
      itemCount: 3,
      total: demoProducts[0]!.price * 2 + demoProducts[3]!.price,
    });
  });
  ...
  it("removes and clears lines", () => {
    ...
    expect(
      cartReducer(state, { type: "remove", productId: product.id }),
    ).toEqual(EMPTY_CART);
