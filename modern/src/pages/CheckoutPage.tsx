...
export function CheckoutPage() {
  ...
  if (state.lines.length === 0) {
    return (
      <section className="state-card page-state">
        <h1>No hay una compra para finalizar</h1>
        <p>El checkout demo necesita al menos un producto.</p>
  ...
  return (
    <section className="checkout-page page-section">
      <div>
        <p className="eyebrow">Checkout educativo</p>
        <h1>Finalizar demo</h1>
        <p>
          Estos datos se usan sólo para construir la confirmación local de esta
          sesión. No se envían al Firebase histórico.
        </p>
      </div>
      <div className="checkout-layout">
        <form className="checkout-form" onSubmit={handleSubmit} noValidate>
          ...
          {status === "error" && (
            <p role="alert" className="form-alert">
              No pudimos registrar la orden demo. Tu carrito sigue intacto;
        ...
        <aside className="order-summary">
          <p className="eyebrow">Total demostrativo</p>
          <h2>{formatMoney(summary.total)}</h2>
          <p>{summary.itemCount} unidades.</p>
          <p className="fine-print">
            No se procesa un pago real ni se promete un email posterior.
          </p>
        </aside>
      </div>
    </section>
